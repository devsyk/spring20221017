...
<body>
	...
	<form action="" id="form3">
		...
		hasCar<input type="checkbox" id="hasCarCheckBox3" value="true"><br>
		food:피자<input type="checkbox" class="foodCheckbox3" value="피자"><br>
		food:햄버거<input type="checkbox" class="foodCheckbox3" value="햄버거"><br>
		food:커피<input type="checkbox" class="foodCheckbox3" value="커피">
	</form>
	<br><br>
	
	<button id="btn14">/ex44/sub14 post json + 날짜(date)</button><br>
...
<script>
	...
	document.querySelector("#btn13").addEventListener("click", function() {
		...
		const hasCar = document.querySelector("#hasCarCheckBox3").checked;
		const food = [];
		/*
		const checkedFoods = document.querySelectorAll(".foodCheckbox3");
		for(let i=0; i<checkedFood.length; i++) {
			if(checkedFood[i].checked) {
				food.push(checkedFood[i].value);
			}
		}
		*/
		const checkedFoods = document.querySelectorAll(".foodCheckbox3:checked");
		for(const checkedFood of checkedFoods) {
			food.push(checkedFood.value);
		}
		/*
		const obj = {
			age : age,
	...
	document.querySelector("#btn8").addEventListener("click", function() {
		...
		const data = JSON.stringify(o); // 객체 -> String JSON
	...
	document.querySelector("#btn5").addEventListener("click", function() {
		fetch(ctx + "/ex44/sub05", {
			...
			body : `{"address":"seoul", "score":88, "phone":["a", "b"], "married":true}`
		});
	});
